...
function MetricCard({ metric }: { metric: DashboardMetric }) {
  ...
  return (
    <Card className="group border-white/70 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
...
export function CommandDeckView({
  ...
  return (
    <div className="space-y-5">
      <section className="flex flex-col gap-4 rounded-2xl border border-emerald-100 bg-gradient-to-r from-emerald-50 to-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="สถานะสำคัญ">{viewModel.metrics.map((metric) => <MetricCard key={metric.id} metric={metric} />)}</section>
