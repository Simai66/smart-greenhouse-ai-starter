...
import { Archive, BellRing, Bot, Camera, Clock3, Cpu, Droplets, Pencil, Plus, RotateCcw, Settings2, Sparkles, Trash2 } from "lucide-react";
...
import { LiveSensorSection } from "@/components/greenhouse/settings/live-sensor-section";
...
import type { DemoCamera, DemoCropBatch, DemoDevice, DemoGreenhouse, DemoPlant, DemoSensor, DemoSettings } from "@/lib/greenhouse-demo-store";
...
function CropBatchesSection({ greenhouses, cropBatches, plants, activeGreenhouseId, onSaveBatch, onArchiveBatch, onRestoreBatch, onPermanentlyDeleteBatch }: {
  ...
  onPermanentlyDeleteBatch: (greenhouseId: string, id: string) => void;
}) {
  const [editing, setEditing] = useState<DemoCropBatch | null | undefined>(undefined);
  const [deleting, setDeleting] = useState<DemoCropBatch | null>(null);
  const [deleteError, setDeleteError] = useState("");
  const activeGreenhouse = greenhouses.find((greenhouse) => greenhouse.id === activeGreenhouseId);
  ...
  const deletedPlantCount = deleting ? plants.filter((plant) => plant.greenhouseId === deleting.greenhouseId && plant.batchId === deleting.id).length : 0;
  const confirmPermanentDelete = () => {
    if (!deleting) return;
    try {
      onPermanentlyDeleteBatch(deleting.greenhouseId, deleting.id);
      setDeleting(null);
      setDeleteError("");
    } catch (error) {
      setDeleteError(error instanceof Error ? error.message : "ไม่สามารถลบรอบปลูกถาวรได้");
    }
  };
  ...
  return <>
    ...
    <Dialog open={Boolean(deleting)} onOpenChange={(open) => { if (!open) { setDeleting(null); setDeleteError(""); } }}><DialogContent><DialogHeader><DialogTitle>ยืนยันการลบรอบปลูกถาวร</DialogTitle><DialogDescription>{deleting ? `จะลบ “${deleting.cropName}” ใน ${zoneName(deleting)} ที่ปลูกเมื่อ ${new Intl.DateTimeFormat("th-TH", { dateStyle: "medium" }).format(new Date(`${deleting.plantedAt}T00:00:00`))} และข้อมูลพืช ${deletedPlantCount} ต้นอย่างถาวร ไม่สามารถเรียกคืนได้` : ""}</DialogDescription></DialogHeader>{deleteError ? <p className="text-sm text-destructive" role="alert">{deleteError}</p> : null}<DialogFooter><Button type="button" variant="outline" onClick={() => setDeleting(null)}>ยกเลิก</Button><Button type="button" variant="destructive" onClick={confirmPermanentDelete}>ลบถาวร</Button></DialogFooter></DialogContent></Dialog>
  </>;
}
...
function ResourceBindingsSection({ greenhouses, activeGreenhouseId, devices, cameras, sensors, onCreateResource, onMoveResource, onRenameResource, onUpdateResourceStatus, onDeleteResource }: {
  ...
  {editing || creatingKind ? <ResourceEditorDialog mode={editing ? "edit" : "create"} zones={zones} initialValue={editorValue} onOpenChange={(open) => { if (!open) { setEditing(null); setCreatingKind(null); } }} onSubmit={submitEditor} /> : null}
...
export function SettingsView({ settings, greenhouses, cropBatches, plants, activeGreenhouseId, devices, sensors, liveSensors, liveSensorsLoading, liveSensorsError, liveSensorsFetchedAt, onRefreshLiveSensors, onSave, onSaveGreenhouse, onArchiveGreenhouse, onRestoreGreenhouse, onAddZone, onRenameZone, onArchiveZone, onRestoreZone, onPermanentlyDeleteGreenhouse, onPermanentlyDeleteZone, onSaveBatch, onArchiveBatch, onRestoreBatch, onPermanentlyDeleteBatch, onCreateResource, onMoveResource, onRenameResource, onUpdateResourceStatus, onDeleteResource }: { settings: DemoSettings; greenhouses: DemoGreenhouse[]; cropBatches: DemoCropBatch[]; plants: DemoPlant[]; activeGreenhouseId: string; devices: DemoDevice[]; sensors: DemoSensor[]; liveSensors: import("@/lib/sensor-api").LiveSensor[]; liveSensorsLoading: boolean; liveSensorsError: string; liveSensorsFetchedAt: string | null; onRefreshLiveSensors: () => Promise<void>; onSave: (settings: DemoSettings) => void; onSaveGreenhouse: (id: string | null, draft: GreenhouseDraft) => void; onArchiveGreenhouse: (id: string) => void; onRestoreGreenhouse: (id: string) => void; onAddZone: (greenhouseId: string, name: string) => void; onRenameZone: (greenhouseId: string, zoneId: string, name: string) => void; onArchiveZone: (greenhouseId: string, zoneId: string) => void; onRestoreZone: (greenhouseId: string, zoneId: string) => void; onPermanentlyDeleteGreenhouse: (id: string) => void; onPermanentlyDeleteZone: (greenhouseId: string, zoneId: string) => void; onSaveBatch: (id: string | null, draft: CropBatchDraft) => void; onArchiveBatch: (id: string) => void; onRestoreBatch: (id: string) => void; onPermanentlyDeleteBatch: (greenhouseId: string, id: string) => void; onCreateResource: (value: ResourceEditorValue) => void; onMoveResource: (kind: "device" | "camera" | "sensor", id: string, zoneId: string) => void; onRenameResource: (kind: "device" | "camera" | "sensor", id: string, name: string) => void; onUpdateResourceStatus: (kind: "device" | "camera" | "sensor", id: string, value: Pick<ResourceEditorValue, "enabled" | "status">) => void; onDeleteResource: (kind: "device" | "camera" | "sensor", id: string) => void }) {
  ...
  return <div className="space-y-6">
    <FarmStructureSection greenhouses={greenhouses} onSaveGreenhouse={onSaveGreenhouse} onArchiveGreenhouse={onArchiveGreenhouse} onRestoreGreenhouse={onRestoreGreenhouse} onAddZone={onAddZone} onRenameZone={onRenameZone} onArchiveZone={onArchiveZone} onRestoreZone={onRestoreZone} onPermanentlyDeleteGreenhouse={onPermanentlyDeleteGreenhouse} onPermanentlyDeleteZone={onPermanentlyDeleteZone} />
    <CropBatchesSection greenhouses={greenhouses} cropBatches={cropBatches} plants={plants} activeGreenhouseId={activeGreenhouseId} onSaveBatch={onSaveBatch} onArchiveBatch={onArchiveBatch} onRestoreBatch={onRestoreBatch} onPermanentlyDeleteBatch={onPermanentlyDeleteBatch} />
    ...
    <LiveSensorSection key={activeGreenhouseId} greenhouseId={activeGreenhouseId} sensors={liveSensors} loading={liveSensorsLoading} error={liveSensorsError} fetchedAt={liveSensorsFetchedAt} onRefresh={onRefreshLiveSensors} />
