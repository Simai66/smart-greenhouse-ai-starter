...
import type { DemoGreenhouse, DemoZone } from "@/lib/greenhouse-demo-store";
...
type FarmStructureSectionProps = {
  ...
  onPermanentlyDeleteGreenhouse: (id: string) => void;
  onPermanentlyDeleteZone: (greenhouseId: string, zoneId: string) => void;
};

type DeleteTarget =
  | { kind: "greenhouse"; greenhouse: DemoGreenhouse }
  | { kind: "zone"; greenhouse: DemoGreenhouse; zone: DemoZone };
...
export function FarmStructureSection({ greenhouses, onSaveGreenhouse, onArchiveGreenhouse, onRestoreGreenhouse, onAddZone, onRenameZone, onArchiveZone, onRestoreZone, onPermanentlyDeleteGreenhouse, onPermanentlyDeleteZone }: FarmStructureSectionProps) {
  ...
  const confirmPermanentDelete = () => {
    if (!deleting) return;
    try {
      if (deleting.kind === "greenhouse") onPermanentlyDeleteGreenhouse(deleting.greenhouse.id);
      else onPermanentlyDeleteZone(deleting.greenhouse.id, deleting.zone.id);
      setDeleting(null);
      setDeleteError("");
    } catch (error) {
      setDeleteError(error instanceof Error ? error.message : "ไม่สามารถลบถาวรได้");
    }
  };
  ...
  return <>
    ...
    <Dialog open={Boolean(deleting)} onOpenChange={(open) => { if (!open) { setDeleting(null); setDeleteError(""); } }}><DialogContent><DialogHeader><DialogTitle>ยืนยันการลบถาวร</DialogTitle><DialogDescription>{deleting ? `จะลบ “${deleting.kind === "greenhouse" ? deleting.greenhouse.name : deleting.zone.name}” อย่างถาวรและไม่สามารถเรียกคืนได้` : ""}</DialogDescription></DialogHeader>{deleteError ? <p className="text-sm text-destructive" role="alert">{deleteError}</p> : null}<DialogFooter><Button type="button" variant="outline" onClick={() => setDeleting(null)}>ยกเลิก</Button><Button type="button" variant="destructive" onClick={confirmPermanentDelete}>ลบถาวร</Button></DialogFooter></DialogContent></Dialog>
  </>;
}
