...
lock_directory="${runtime_root}/install.lock"
...
process.stdout.write(createHash("sha256").update(await readFile(process.argv[2])).digest("hex"));
NODE
)"
use_seeded_cache=0
...
if [[ -z "${locked_tarball}" || -z "${locked_integrity}" || "$(printf '%s\n' "${locked_vinext_output}" | wc -l | tr -d ' ')" != "2" ]]; then
